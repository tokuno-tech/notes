...
top-p / top-kは「多様性パラメータ」と呼ばれる（候補トークンの広がりを制御するため）。これを無効化（絞り込みを最小化）すると、出力の変動性が減り、簡潔さや一貫性を強制しやすくなる＝**厳密な長さ制限やブランドルールの適用に有利**。ただしその代償として**創造性が制限される**。「一貫性と創造性の両方が必要」という要件では、多様性パラメータを完全に無効化する設定は創造性を犠牲にしすぎるため不正解になりうる（中程度のtemperatureで確定性とランダム性のバランスを取りつつ、多様性パラメータ自体は維持する方が要件に合う）。
...
### Bedrockはステートレス（多ターン会話でのエラーの根本原因）

```
Amazon Bedrock はリクエスト間で会話状態を保持しない。
多ターン会話を続けるには、アプリ側が過去の user/assistant メッセージを
毎回まるごと messages 列に含めて送信する必要がある

→ 会話が長引くほど送信するメッセージ列の総トークン数が増える
→ モデル固有の「コンテキストウィンドウ上限」（入力トークンの最大量、モデルごとに固定）を
  超えた時点でエラーが発生する
→ 「会話が長いユーザーほどエラーが出る」「ユーザーごとにエラー発生タイミングが違う」
  という症状は、この蓄積が原因であるサイン
```

### CountTokens API（推論前のトークン数計測）

- 実際に推論リクエストを送信する**前に**、入力（messagesやsystemプロンプト含む）のトークン数を計測できるAPI
- **推論を実行しないため課金されない**
- Converse APIのリクエスト形式に対応。対応モデル・リージョンは限定的なので事前確認が必要
- 使い方：閾値に近づいたら、会話の先頭（古いターン）を**要約した短いメッセージに置き換えて**から送信 → コンテキストサイズを上限内に維持しつつ文脈は残せる（プルーニング/要約置き換え）

### ⚠️ `maxTokens`（inferenceConfig）は入力側の上限とは無関係（誤答しやすい）

```
inferenceConfig.maxTokens
  = モデルが「生成する応答（出力）」のトークン数上限を制御する推論パラメータ
  ≠ モデルが「受け付けられる入力」の上限（コンテキストウィンドウ）

コンテキストウィンドウ（入力上限）はモデルごとに固定されており、
推論パラメータ（maxTokens等）をどう変更しても拡張できない
```

→ 「会話が長引いて入力トークン超過エラーが出た」場面で`maxTokens`を引き上げる選択肢は的外れ（出力側のパラメータをいじっても入力側の上限は変わらない）

### Guardrailsは「コンテキストウィンドウ超過」対策ではない

Guardrailsの設計目的はPII検出・有害コンテンツフィルタリング・禁止トピック拒否・グラウンディング確認等の**コンテンツ安全性・ガバナンス**であり、入力長超過の検知・トリミングは設計対象外（そのようなポリシー設定機能もない）。

### Compaction機能（ベータ・参考情報）

Bedrockには会話が長くなった際にコンテキストを自動で要約・圧縮する**Compaction機能（ベータ）**が存在するが、2026年時点では**InvokeModel APIからの呼び出し限定、かつClaude Opus/Sonnetモデルのみ対応**。**Converse APIでは利用できない**点に注意（Converse API使用のシナリオでは使えない選択肢になる）。
...
## モデルカスタマイズ手法の比較（継続事前学習・ファインチューニング）

| 種類 | 学習データ形式 | 目的 |
|---|---|---|
| **継続事前学習（Continued Pre-training / CPT）** | 大量のテキスト（ラベルなし） | ドメイン知識・専門語彙を追加学習 |
| **通常のファインチューニング** | 入力→出力のペア | 特定タスクへの特化 |
| **インストラクションベースのファインチューニング** | 指示→応答のペア（`{"instruction":..., "input":..., "output":...}`） | 指示に従う能力を強化 |

- Bedrockのカスタムモデル機能で対応。学習データはS3に置いたJSONL形式
- **CPTはAmazon Titan Textモデルのみが対象**（Claude等サードパーティFMには適用不可）
- CPTはドメイン語彙・概念の統計的分布を学習させる手法であり、**特定ドキュメントの具体的な数値・条件を推論時に動的参照する能力は付与されない**（診療報酬点数のような事実情報の正確な参照にはRAGが必要、CPTでは代替不可。Udemy問9で確認）
- 「毎月の情報更新に追従」要件が出たらCPT/ファインチューニングの再実行はいずれも**再学習のタイムラグ＋コスト**で不正解、**RAG（Knowledge Bases）が事実情報の頻繁な更新への一択**
- **注意（2026-07-08、公式ドキュメント裏取り）**：`custom-models.html`（Bedrockのモデルカスタマイズ手法の公式一覧ページ）には現在 **Supervised fine-tuning／Reinforcement fine-tuning／Distillation** の3つのみ掲載で、「継続事前学習」という項目名は見当たらない（専用ページも見つからずリダイレクトされた）。ただしUdemy問9でTitan Text限定のCPTジョブとして出題されており実在は確認できている。「ラベルなしデータ→継続事前学習」の判断ルール自体は試験対策として使ってよい
...
### 「Hugging Faceフォーマット」の実体（ファイル拡張子）

「Hugging Faceフォーマット」は単一の拡張子ではなく、Hugging Face transformersライブラリが出力する**ファイル一式のレイアウト**を指す。S3から直接インポートするために最低限必要なファイル：

| ファイル | 役割 | 拡張子 |
|---|---|---|
| モデルの重み本体 | ニューラルネットの数値パラメータ | `.safetensors` |
| `config.json` | アーキテクチャ設定（隠れ層数・attention head数など） | `.json` |
| `tokenizer_config.json` / `tokenizer.json` / `tokenizer.model` | トークナイザー | `.json` / `.model` |

Safetensorsは**Hugging Faceが作った重み保存形式そのものの名前**（拡張子`.safetensors`）。従来のPyTorch `.bin`（pickleベース）は読み込み時に任意コード実行の脆弱性があったため、それに代わる安全な形式として設計された。

対応アーキテクチャは限定的（Llama系、Mistral、Mixtral、Flan、GPTBigCode、Qwen系、GPT-OSS等）。埋め込みモデルは非対応、モデルサイズはテキストモデルで200GB未満、マルチモーダルで100GB未満の制限あり。
