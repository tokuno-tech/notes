...
### 複数Bedrockエージェントの順次実行タスク粒度（Udemy問1）

Step Functionsで複数のBedrockエージェント＋Lambdaを順次実行する構成を組む場合、**タスクの分割粒度**によってエラーハンドリングの柔軟性が変わる。

- 各エージェント／Lambdaを**個別のTaskステート**として定義すると、タスクごとに個別のRetryとCatchを設定できる。障害発生箇所ごとに異なるフォールバック（デフォルト値・キャッシュ応答等）を組める
- 複数の呼び出しを1つのLambda関数や1つのTaskステートにまとめると、そのブロック単位でしかRetry/Catchを設定できず、エラーハンドリングの粒度が粗くなる
- Step FunctionsはBedrock Agentsの`InvokeAgent`とネイティブ統合（Optimized Integration）しているため、Lambda経由せず直接呼び出せる（上記「ネイティブ統合サービス一覧」参照）
