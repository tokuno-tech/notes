...
## CloudWatch サービス全体マップ

### ユーザー体験の監視

| サービス | 分類 | 役割 / 使い分け |
|---------|------|---------------|
| **RUM**（Real User Monitoring） | 実ユーザー | 実際のユーザーのブラウザから直接計測。ページロード時間・クライアントエラー・離脱ポイント。問題発生と**同時**に検知。 |
| **Synthetics**（Canary） | 模擬ユーザー | スクリプトで定期的に模擬操作（外形監視）。ユーザー到達前に劣化を**先回り**検知。 |

> RUM vs Synthetics：実ユーザーの体験を測るか、先回りで監視するかの違い
...
| サービス | 分類 | 役割 / 使い分け |
|---------|------|---------------|
| **CloudWatch Logs** | ログ | アプリ・AWSサービスのログを収集・保存。Bedrock の呼び出しログもここへ。 |
| **Logs Insights** | ログ分析 | CloudWatch Logs を SQL ライクにクエリ。高レイテンシー・エラー集計・特定プロンプトの横断検索。 |
| **機密データ保護**（Sensitive Data Protection） | PII マスク | ログに含まれる PII（氏名・電話番号等）を保存時に自動マスク。プロンプトログの個人情報漏洩対策。 |

> Logs は「保存」、Logs Insights は「検索・集計」。セットで使う。
> 機密データ保護 vs Comprehend：Comprehend＝処理前検出、機密データ保護＝ログ保存時マスク
...
| サービス | 分類 | 役割 / 使い分け |
|---------|------|---------------|
| **CloudWatch Metrics** | メトリクス | 数値データの時系列保存。レイテンシー・エラー率・トークン数など。カスタムメトリクスも送信可。 |
| **EMF**（Embedded Metric Format） | メトリクス | ログに JSON を埋め込むだけでメトリクスを同時生成。Lambda からカスタムメトリクスを手軽に送れる。 |
| **Alarms**（アラーム） | アラーム | メトリクスが固定しきい値を超えたら通知・アクションをトリガー。 |
| **複合アラーム**（Composite Alarm） | アラーム | 複数のアラームを AND/OR で組み合わせ。「エラー率↑ かつ レイテンシー↑ の時だけ通知」など誤報削減。 |
| **異常検出アラーム**（Anomaly Detection） | AI アラーム | ML で正常範囲を自動学習し逸脱したらアラーム。**基準値が自動更新される**のが特徴。 |
| **Contributor Insights** | ランキング | 「どの IP / ユーザー / リソースが一番多いか」をリアルタイムランキング。「最多エラー源」の特定に。 |
| **詳細モニタリング**（Detailed Monitoring） | 収集間隔 | メトリクス収集を基本の**5分間隔→1分間隔**に短縮（有料）。EC2/RDS/OpenSearch Service等で有効化可。短時間の変動・スパイクを細かく捉えたい時に使う。それ自体は根本原因の特定手段ではない点に注意。 |

> 異常検出＝「**いつ**」異常か、Contributor Insights＝「**何が**」一番多いか
...
| サービス | 分類 | 役割 / 使い分け |
|---------|------|---------------|
| **X-Ray** | 分散トレース | マイクロサービス間のリクエストを追跡。どのステップで遅延・失敗が起きたかを可視化。サービスマップを自動生成。 |
| **ServiceLens** | 統合ビュー | CloudWatch（ログ・メトリクス）＋ X-Ray（トレース）を1画面で確認。MTTR短縮が目的。 |
| **Application Signals** | SLO管理 | ServiceLens の後継。SLO（目標値）を設定し、SLI（計測値）が違反しそうなときに事前アラート。 |

> ServiceLens＝障害対応（MTTR短縮）、Application Signals＝SLO管理で品質を維持
...
| サービス | 分類 | 役割 / 使い分け |
|---------|------|---------------|
| **Container Insights** | コンテナ | ECS / EKS / Kubernetes のメトリクスを自動収集。CPU・メモリ・ネットワークをコンテナ単位で可視化。 |
| **Application Insights** | EC2 アプリ | EC2 上の .NET / SQL Server 等のアプリを監視。ML でベースラインを自動設定。 |

> Container Insights はコンテナ専用。EC2 アプリの監視は Application Insights を使う。
...
| サービス | 分類 | 役割 / 使い分け |
|---------|------|---------------|
| **GenAI Observability**（AgentCore タブ） | GenAI | Bedrock AgentCore のエージェントを横断監視。セッション数・実行時間・エラー率・ツール使用状況を一覧表示。 |
| **事前設定済みビュー** | GenAI | ダッシュボード構築不要で GenAI ワークロードをすぐ可視化。レイテンシー・トークン使用量・エラー率。 |
...
**OpenTelemetry の AWS 公式ディストリビューション**。OSSの OpenTelemetry を AWS 環境向けに最適化したもの。

- **目的**：Bedrock 以外（OpenAI / SageMaker / オンプレモデル等）のサードパーティモデルのトレース・メトリクスを CloudWatch に統一して送信する
- **仕組み**：アプリに ADOT SDK を組み込むと、モデル呼び出しのトレースが自動的に OpenTelemetry 形式に変換され、CloudWatch の OTLP エンドポイントへ送信される
- **メリット**：Bedrock モデルとサードパーティモデルが混在するシステムでも、CloudWatch 1か所で横断監視できる

```
Bedrock モデル        → enableTrace / AgentCore Observability → CloudWatch
サードパーティモデル  → ADOT SDK（OTEL形式に変換）          → CloudWatch
```

→ 試験で「Bedrock 外のモデルのトレースを CloudWatch に統合したい」文脈が出たら **ADOT SDK** が正解

### CloudWatch 事前設定済みビュー（Task 5.2）

CloudWatch は GenAI ワークロード向けに**ダッシュボードを自前構築しなくても使える事前設定済みのビュー**を提供している。

- レイテンシー・トークン使用量・エラー率がすぐに可視化できる
- 試験で「迅速に可視化・監視したい」文脈 → 事前設定済みビューが正解候補

### CloudWatch GenAI オブザーバビリティ（Domain 5 ボーナス5）

GenAI アプリの**監視・トラブルシューティング専用**機能。Bedrock と連携する Lambda 等から**プロンプト実行データとモデル応答を自動収集・分析**し、最小セットアップで使える（事前設定済みビューはこの一部）。

**何を可視化できるか（対応ソース）**
- Bedrock AgentCore エージェント（マネージド/セルフホスト/サードパーティ）、モデル呼び出し、ナレッジベース、ガードレール、ツール
- AWS Strands / LangChain / LangGraph などのオーケストレーションフレームワークに対応（フレームワーク非依存）

**見られる指標・データ**
- モデル呼び出しダッシュボード：トークン消費、推論の入出力コンテンツ
- エージェントダッシュボード：エージェント・メモリ・ツール・ゲートウェイのパフォーマンス/決定メトリクス
- 合計/平均呼び出し数、トークン使用量（入力/出力/平均）、レイテンシ（平均/P90/P99）、エラー率・スロットリング、応答品質、コスト属性（アプリ/ユーザー別）
- **エンドツーエンドのプロンプトトレーシング**で「ナレッジベース・ツール・モデル」のどこに問題があるか特定

**前提・他サービスとの関係**
- サードパーティモデルの構造化トレースは **ADOT SDK** で CloudWatch に送信（→ [ADOT SDK](#) は別項参照）
- **Application Signals / X-Ray との違い**：Application Signals・X-Ray は汎用の分散トレース・サービスマップ（リクエストフロー/タイミングの可視化）で、**GenAI特化ではない**。プロンプト⇔応答の関係やモデルパフォーマンスのパターン分析は持たず、トレース設定・解釈で運用オーバーヘッドが増える
- 試験で「**デプロイ後のGenAIアプリでプロンプト応答が不安定 → 最小オーバーヘッドで監視・トラブルシュート**」→ **GenAI オブザーバビリティ**が正解（Application Signals/X-Ray は汎用すぎて不正解側）

### VectorDB のギャップ（Task 5.2）

RAG パイプラインで「検索はできているのに FM の回答が不正確」な場合の原因パターン。

```
ユーザー質問 → ベクトル検索 → ヒットなし or 低スコア（= ギャップ）
                                ↓
                    FM への入力コンテキストが空 or 不十分
                                ↓
                    ハルシネーション・不正確な回答
```

**原因：** 埋め込みモデルとクエリのドメインミスマッチ、インデックス未更新、チャンキング不備
**診断：** CloudWatch の接続ビューで検索ステップのヒット数・スコアを確認

### CloudWatch Logs 機密データ保護（Sensitive Data Protection）（Task 5.2）

CloudWatch Logs の機能で、ログに含まれる PII（個人情報）を自動検出してマスクする。

- GenAI のプロンプト・応答ログにユーザーの個人情報が混入するリスクに対応
- マスク対象：メールアドレス・電話番号・クレジットカード番号・氏名 等

```
ログ原文: "田中太郎（090-XXXX-XXXX）の予約を確認してください"
マスク後: "***（***-****-****）の予約を確認してください"
```

→ 試験で「プロンプトログの PII 漏洩を防ぎたい」文脈 → **CloudWatch Logs 機密データ保護**が正解候補
→ Comprehend（入力前検出・マスク）とは役割が異なる：Comprehend=処理前、機密データ保護=ログ保存時

---

---

## OSS 監視ツール（Prometheus / Grafana）
...
JVMMemoryPressure
  85% 超 → GC(ガベージコレクション) 頻発 → レイテンシー急増の前兆
...
## トレース・サービスマップ（X-Ray / ServiceLens / Application Signals）
...
| 用語 | 正式名 | 意味 |
|------|--------|------|
| **SLI** | Service Level Indicator | 実際に計測する指標（例：レイテンシー・エラー率） |
| **SLO** | Service Level Objective | チーム内で設定する目標値（例：p99 レイテンシー < 500ms） |
| **SLA** | Service Level Agreement | 顧客との契約上の約束（SLO より厳しく設定することが多い） |

```
SLI（計測値）= p99 レイテンシー 450ms
SLO（目標）  = p99 < 500ms  ← Application Signals で管理
SLA（契約）  = p99 < 1000ms ← 守れないと違約金等
```

→ CloudWatch Application Signals は SLO を設定し、SLI が違反しそうなときにアラートを出す機能を持つ
