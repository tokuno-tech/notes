...
### 試験での判断軸（AIP-66 パターン）
```
「数千件の小規模データ」+ 「精度最優先（100% recall）」+ 「低レイテンシー」
  → MemoryDB Flat ✅
  
  理由：
  - 小規模ならFlatでも総当たりコストは低い
  - Flatはブルートフォース = 完全最近傍 = recall 100%保証
  - MemoryDBはインメモリ → ディスクアクセスなし → 低レイテンシー

「大規模データ」+「速度優先」
  → HNSW（MemoryDB or OpenSearch）✅
  → ただし近似検索のためrecall 100%は保証されない
```

### 各サービスの特性まとめ

| サービス | インデックス | 検索方式 | Recall | レイテンシー |
|---|---|---|---|---|
| **MemoryDB Flat** | Flat | Exact NN | **100%** | 低（インメモリ） |
| **MemoryDB HNSW** | HNSW | ANN | < 100% | 低（インメモリ） |
| **OpenSearch HNSW** | HNSW | ANN | < 100% | 中（ディスク） |
| **Aurora pgvector HNSW** | HNSW | ANN | < 100% | 高（ディスクベース） |
| **Neptune Database** | なし | **非対応** | N/A | N/A |
