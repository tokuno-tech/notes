...
## SQSキューイング（Bedrock FMの前段）（6/9 学習分）
...
## AWS PrivateLink（Bedrock/SageMaker推論）（6/9 学習分）
...
**試験：「セキュリティ」「低レイテンシ」「プライベート通信」→ PrivateLink**

---

## プロビジョンドスループット vs オンデマンド（6/9 学習分）
...
## Bedrock Agent セッションメモリ（sessionId）（6/9 学習分）
...
| キーワード | 正解 |
|---|---|
| 「エージェントで複数ターンのコンテキストを維持」 | **Bedrock Agent sessionId** |
| 「追加実装なし・マネージドで」 | **Bedrock Agent sessionId（DynamoDBは追加実装が必要）** |
| 「自前でFMを呼ぶアプリでマルチターン」 | **messages[]自己管理 or DynamoDB** |

---

## AWS Strands Agents / Agent Squad（6/9 学習分）
...
```
「マネージドで素早く構築」→ Bedrock Agents
「コードで細かく制御」「マルチプロバイダ」→ AWS Strands Agents
「複数の専門エージェントを用途に応じて振り分け」→ Agent Squad
```

---

## MCP（Model Context Protocol）（6/8〜6/9 学習分）
...
| キーワード | 正解 |
|---|---|
| 「複数FMで同じツールを使いたい」「標準化」 | **MCP** |
| 「プロトコルの複雑さを隠蔽」「抽象化」 | **MCPクライアントライブラリ** |
| 「FMを変えてもツール接続を変えたくない」 | **MCP（相互運用性）** |
