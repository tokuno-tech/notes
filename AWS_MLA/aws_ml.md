...
### 方針
- 4日後試験、本番で72%を割らない状態を作る
- 新規問題を大量に解くより、間違えた問題の判断ルールを固定
- 優先：**デプロイとオーケストレーション**
- データ準備は問題数少ないが、Glue / Data Wrangler / Feature Store / Clarify / Ground Truth 周辺を軽く確認
- Udemy模試は悪問・曖昧問題も混じるため、点数を実力評価にしすぎない
...
| 知識不足 | 用語・サービスの役割をメモ |
| 読み違い | 問題文の条件を確認 |
| 判断ミス | 選択肢の比較ルールを1行で整理 |
| 悪問気味 | 公式知識だけ回収して深追いしない |

### 復習時に見る観点
- なぜ正解がそれなのか
- 自分の選択肢はどの条件なら正解になるのか
- 似ているサービスとの違いは何か
- 問題文のどのキーワードで判断すべきだったか

### 最終方針
- 直前は知識を増やすより**判断ルールを固定**
- 迷ったら「問題文の制約」に戻る
- サービス名だけで選ばず、**目的・入力・出力・運用負荷**で比較

---

## 一言まとめ
SageMakerは「開発 → 実験 → 訓練 → 管理 → デプロイ → 監視」のML全工程をカバーする
